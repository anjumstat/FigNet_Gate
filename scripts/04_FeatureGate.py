# -*- coding: utf-8 -*-
"""
PAPER 2: Feature Importance Gate for Interpretable Enzyme Classification

Output workflow updated to match PAPER 1 corrected script:
...
Paper 2 focus remains the same:
- Feature Importance Gate for interpretable enzyme classification
- Combinations with Real Variational Dropout, Adaptive Variational Dropout, and Dynamic Sparsity
- Feature stability and top-feature analysis

Models:
- 5 VARDON feature-gate variants:
  1) VARDON_FeatureGate
  2) VARDON_Gate_RealVD
  3) VARDON_Gate_AdaptiveVD
  4) VARDON_Gate_Sparsity
  5) VARDON_Gate_Full
- 2 baselines:
  1) Logistic_Regression / Linear Softmax Classifier
  2) MLP_Baseline
"""
...
BASE_DIR = r"D:\zebfish1\VARDON_GATE_Results"
os.makedirs(BASE_DIR, exist_ok=True)

# Kept from your Paper 2 script.
...
METHODS = {
    ...
    "VARDON_Gate_RealVD": {
        ...
        "description": "Feature Gate + Real Variational Dropout",
    },
    "VARDON_Gate_AdaptiveVD": {
        "type": "feature_gate",
        "variant": "gate_adaptive_vd",
        "description": "Feature Gate + Adaptive Variational Dropout",
    },
    "VARDON_Gate_Sparsity": {
        "type": "feature_gate",
        "variant": "gate_sparsity",
        "description": "Feature Gate + Dynamic Sparsity",
    },
    "VARDON_Gate_Full": {
        "type": "feature_gate",
        "variant": "gate_full",
        "description": "Feature Gate + All Components",
    },
...
class FeatureImportanceGate(layers.Layer):
    ...
    The original Paper 2 script used a hard top-k mask. This version keeps the
    intended feature-gate behavior while making the gate smoothly trainable and
    consistent with the corrected Paper 1 script.
    """
...
def build_mlp_model(input_shape, num_classes, learning_rate):
    """Multi-Layer Perceptron baseline from the Paper 2 script."""
    model = models.Sequential([
        layers.Input(shape=(input_shape,)),
...
def build_feature_gate_model(input_shape, num_classes, learning_rate, variant="gate_only"):
    """Build feature gate-focused VARDON variants for Paper 2."""
    inputs = layers.Input(shape=(input_shape,), name="input")
    x = inputs

    # Core Paper 2 component.
...
def build_model(method_name, config, input_shape, num_classes, learning_rate):
    """Factory function for all Paper 2 model types."""
    ...
    if config["type"] == "feature_gate":
        return build_feature_gate_model(input_shape, num_classes, learning_rate, config["variant"])
...
def run_final_test_evaluation(best_result, X_train_val_raw, y_train_val, X_test_raw, y_test, test_indices):
    ...
    pd.DataFrame([final_result]).to_csv(
        os.path.join(BASE_DIR, "Paper2_Final_Independent_Test_Result.csv"),
        index=False,
    )
    ...
    pd.DataFrame([combined_cv_test_result]).to_csv(
        os.path.join(BASE_DIR, "Paper2_Selected_Model_CV_and_Test_Result.csv"),
        index=False,
    )
...
def run_all_models_test_evaluations(results_list, X_train_val_raw, y_train_val, X_test_raw, y_test, test_indices):
    ...
    all_test_df.to_csv(os.path.join(BASE_DIR, "Paper2_Test_Results_All_Models.csv"), index=False)

    print()
    print(f"All-model independent test results saved to: {os.path.join(BASE_DIR, 'Paper2_Test_Results_All_Models.csv')}")
    return all_test_df
...
if __name__ == "__main__":
    print("=" * 80)
    print("PAPER 2: FEATURE IMPORTANCE GATE FOR ENZYME CLASSIFICATION")
    print("For Enzyme vs Non-enzyme Gene Classification")
    print("=" * 80)
    print(f"Data path: {DATA_PATH}")
    print(f"Results directory: {BASE_DIR}")
    print(f"Models: {len(METHODS)} total = 5 feature-gate variants + 2 baselines")
    ...
    cv_summary_path = os.path.join(BASE_DIR, "Paper2_CV_Summary_Results.csv")
