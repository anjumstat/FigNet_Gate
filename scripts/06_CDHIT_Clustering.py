# -*- coding: utf-8 -*-
"""
UniProt Data Processor for 12 Fish Species
Modified from original plant enzyme classification pipeline
"""
...
# =============================================
# 1. Configuration for Fish Species
# =============================================

class Config:
    DATA_DIR = r"D:\zebfish\data"
    OUTPUT_DIR = r"D:\zebfish\data\processed_results_v3"
    
    # Your 12 fish species mapping
    SPECIES = {
        '8030': {  # Atlantic salmon
            'name': 'Atlantic_salmon',
            'folder': 'atlansalmon'
        },
        '7998': {  # Channel catfish
...
class UniProtDataProcessor:
    ...
    def find_files_in_folder(self, folder_path):
        """Find TSV, HDF5, and FASTA files in a folder"""
        tsv_file = None
        h5_file = None
        fasta_file = None
        ...
        # If no FASTA found, look for compressed or alternative names
        if fasta_file is None:
            for file in os.listdir(folder_path):
                if file.endswith('.fasta.gz') or file.endswith('.fa.gz'):
                    gz_path = os.path.join(folder_path, file)
                    fasta_file = self.decompress_file(gz_path)
                    if fasta_file:
                        print(f"    📄 Decompressed FASTA: {os.path.basename(fasta_file)}")
                    break
        
        return tsv_file, h5_file, fasta_file
    ...
    def classify_enzymes(self, df):
        ...
        # Show sample values from EC column
    ...
    def process_species(self, tax_id, species_info):
        ...
        # Find files
        tsv_path, h5_path, fasta_path = self.find_files_in_folder(folder_path)
        ...
        if not fasta_path:
            print(f"⚠️ No FASTA file found in {folder_path} (will use random splits later)")
            print(f"   Files in folder: {os.listdir(folder_path)}")
    ...
    def process_all_species(self):
        """Process all species"""
        print("=" * 80)
        print("UNIPROT DATA PROCESSOR - 12 FISH SPECIES")
        print("=" * 80)
    ...
    def generate_report(self, combined_df, species_stats):
        ...
        with open(report_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("UNIPROT DATA PROCESSING REPORT - 12 FISH SPECIES\n")
            f.write("=" * 80 + "\n")
...
def main():
    print("=" * 80)
    print("FISH SPECIES UNIPROT DATA PROCESSOR")
    print("=" * 80)
